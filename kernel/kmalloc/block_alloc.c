...
int Balloc_Initialize() {
    ...
    FindFreeVirtualAddress(
        GetActiveVirtualMemoryInstance(), &virtBaseAddr_base, SMALL_HEAP_MEM_SIZE,
    ...
    uint64_t phys_addr = AllocatePhysicalPageCont(
                             SMALL_HEAP_MEM_SIZE / PAGE_SIZE, PhysicalMemoryAllocationFlags_None);

    MapPage(GetActiveVirtualMemoryInstance(), phys_addr, virtBaseAddr_base,
            SMALL_HEAP_MEM_SIZE, CachingModeWriteBack, MemoryAllocationType_Heap,
            MemoryAllocationFlags_Kernel | MemoryAllocationFlags_Write |
...
UID _allocBlockSet(size_t startIndex, size_t blockCount, uint8_t *blockData,
                   size_t blockEntries, void *baseLoc, size_t blockSize) {
    // Find the first free block, mark it as a start block
    UID blockIndex = startIndex;

    for (; blockIndex < blockEntries && blockData[blockIndex] != BLOCK_TYPE_FREE;
            blockIndex++)
        ;
    if ((blockIndex == blockEntries) | (blockIndex + blockCount > blockEntries))
        return -1;  //-1 = Not enough memory
    ...
    for (UID i = 0; i < blockCount; i++) {
        if (blockData[blockIndex + i] != BLOCK_TYPE_FREE) {
            ...
            return _allocBlockSet(blockIndex + i + 1, blockCount, blockData,
                                  blockEntries, baseLoc, blockSize);
        }
    }

    // We know we have enough space, quickly set all the mid blocks to MID
    if (blockCount > 2)
        memset(&blockData[blockIndex + 1], BLOCK_TYPE_ALLOC_MID,
               (blockCount - 2) * sizeof(BLOCK_TYPE));

    // Setup the start and end of each block
    blockData[blockIndex] = BLOCK_TYPE_ALLOC_START;
    if (blockCount >= 2)
        blockData[blockIndex + blockCount - 1] = BLOCK_TYPE_ALLOC_END;
    else
        blockData[blockIndex] = BLOCK_TYPE_ALLOC_START_END;

    return blockIndex;
}

UID Balloc_Alloc(size_t size) {
    UID bid = 0;
    ...
    if (size >= LARGE_HEAP_BLOCK_SIZE) {
        ...
        bid = _allocBlockSet(0, blockCount, _largeH_Blocks, LARGE_HEAP_BLOCK_COUNT,
                             _largeH_Loc, LARGE_HEAP_BLOCK_SIZE);
    } else {
        // Use small heap
        // Calculate the blockCount
        ...
        bid = _allocBlockSet(0, blockCount, _smallH_Blocks, SMALL_HEAP_BLOCK_COUNT,
                             _smallH_Loc, SMALL_HEAP_BLOCK_SIZE);
...
void *Balloc_GetBaseAddress(UID blockID) {
    ...
    // If it is a start block calculate its offset assuming that each block before
    // it has a leak check signature
...
UID Balloc_GetUID(void *baseAddress) {
    ...
    if (addr >= (UID)_largeH_Loc &&
            addr < (UID)_largeH_Loc +
            (blockSize + LARGE_HEAP_BLOCK_SIZE) * LARGE_HEAP_BLOCK_COUNT) {
        // Given block is part of the large block allocator
        blockSize += LARGE_HEAP_BLOCK_SIZE;
        return (addr - (UID)_largeH_Loc) / blockSize;
    } else if (addr >= (UID)_smallH_Loc &&
               addr < (UID)_smallH_Loc +
               (blockSize + SMALL_HEAP_BLOCK_SIZE) *
               SMALL_HEAP_BLOCK_COUNT) {
