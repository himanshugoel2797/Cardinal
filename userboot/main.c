...
int LoadProgram(char *name, UID *p_pid) {
    void *elf_loc = NULL;
    size_t elf_sz = 0;
    if (GetFile(name, &elf_loc, &elf_sz)) {
        ...
        *p_pid = pid;
        ...
        return LoadAndStartApplication(pid, elf_loc, elf_sz, name, 1,
                                       strlen(name) + 1);
    }
    return -1;
}

__attribute__((section(".entry_point"))) int _start() {
    if (ImportInitrd() != 0) __asm__("hlt");
    ...
    // Now exit the process, allowing the init process to take control of the
    // system
}