...
bool GetFile(const char *file, void **loc, size_t *size) {
    *loc = NULL;
    *size = 0;
    ...
    while (file_entry != NULL && file_entry->filename[0] != 0) {
        ...
        file_entry =
            (TARHeader *)((uint64_t)file_entry + 512 + getsize(file_entry->size));

        if ((uint64_t)file_entry % 512) {
            file_entry = (TARHeader *)((uint64_t)file_entry +
                                       (512 - (uint64_t)file_entry % 512));
        }
    }
...
int ImportInitrd(void) {
    CardinalBootInfo b_info;
    if (R0_GetBootInfo(&b_info) != 0) return -1;

    uint64_t initrd_addr = 0;
    uint64_t pid = 0;

    if (GetProperty(CardinalProperty_PID, 0, &pid) != 0) return -1;
    ...
    if (R0_Map(pid, initrd_addr, &InitrdStartAddress, InitrdLength,
